...
class TANRModule(AbstractRecommneder):
    """Neural news recommendation with topic-aware news representation.

    Reference: Wu, Chuhan, Fangzhao Wu, Mingxiao An, Yongfeng Huang, and Xing Xie. "Neural news recommendation with topic-aware news representation." In Proceedings of the 57th Annual meeting of the association for computational linguistics, pp. 1154-1159. 2019.

    For further details, please refer to the `paper <https://aclanthology.org/P19-1110/>`_
    ...
    Attributes:
        ...
        dropout_probability:
            Dropout probability.
        topic_pred_loss_coef:
            The weight for the loss of the topic prediction task.
        top_k_list:
    ...
    def __init__(
        ...
        if not self.hparams.use_plm:
            ...
            assert isinstance(self.hparams.num_filters, int)
            assert isinstance(self.hparams.window_size, int)
        ...
        self.news_encoder = NewsEncoder(
            ...
            combine_type=None,
            input_dim=self.hparams.num_filters
            if not self.hparams.use_plm
            else self.hparams.embed_dim,
            query_dim=self.hparams.query_dim,
            output_dim=None,
        )

        # initialize topic predictor
        topic_predictor_in_features = (
            self.hparams.num_filters if not self.hparams.use_plm else self.hparams.embed_dim
        )
        self.topic_predictor = nn.Linear(
            in_features=topic_predictor_in_features, out_features=self.num_categ_classes
        )

        # initialize user encoder, if needed
        if not self.hparams.late_fusion:
            self.user_encoder = UserEncoder(
                news_embed_dim=self.hparams.num_filters
                if not self.hparams.use_plm
                else self.hparams.embed_dim,
                query_dim=self.hparams.query_dim,
            )
    ...
    def forward(self, batch: RecommendationBatch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        topic_scores = self.topic_predictor(torch.cat((cand_news_vector, hist_news_vector), dim=0))
    ...
    def model_step(
        ...
        topic_prob = F.one_hot(topics, num_classes=self.num_categ_classes).type_as(scores)
