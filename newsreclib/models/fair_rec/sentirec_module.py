...
class SentiRecModule(AbstractRecommneder):
    """Sentirec: Sentiment diversity-aware neural news recommendation

    Reference: Wu, Chuhan, Fangzhao Wu, Tao Qi, and Yongfeng Huang. "Sentirec: Sentiment diversity-aware neural news recommendation." In Proceedings of the 1st Conference of the Asia-Pacific Chapter of the Association for Computational Linguistics and the 10th International Joint Conference on Natural Language Processing, pp. 44-53. 2020.

    For further details, please refer to the `paper <https://aclanthology.org/2020.aacl-main.6/>`_
    ...
    Attributes:
        ...
        dropout_probability:
            Dropout probability.
        sent_pred_loss_coef:
            The weight for the loss of the sentiment prediction task.
        sent_div_loss_coef:
            The weight for the loss of the sentiment diversity regularization
        top_k_list:
    ...
    def __init__(
        ...
        sent_pred_loss_coef: float,
        sent_div_loss_coef: float,
        top_k_list: List[int],
        ...
        # initialize sentiment prediction loss
        self.sent_pred_loss = nn.L1Loss()
        ...
        self.sent_predictor = nn.Linear(
            in_features=self.hparams.embed_dim, out_features=self.num_sent_classes
        )
    ...
    def forward(self, batch: RecommendationBatch) -> Tuple[torch.Tensor, torch.Tensor]:
        # encode history
        ...
        sent_scores = self.sent_predictor(torch.cat((cand_news_vector, hist_news_vector), dim=0))

        return scores, sent_scores
    ...
    def model_step(
        ...
        sent_scores = torch.cat(
            (batch["x_cand"]["sentiment_score"], batch["x_hist"]["sentiment_score"])
        )
        sent_pred_loss = self.sent_pred_loss(sent_scores.flatten(), sent_scores)
        loss += self.hparams.sent_pred_loss_coef * sent_pred_loss

        # sentiment diversity regularization loss
        sent_hist, _ = to_dense_batch(batch["x_hist"]["sentiment_score"], batch["batch_hist"])

        sent_cand, _ = to_dense_batch(batch["x_cand"]["sentiment_score"], batch["batch_cand"])
        hist_news_size = torch.tensor(
            [torch.where(mask_hist[n])[0].shape[0] for n in range(mask_hist.shape[0])],
        )
        user_mean_sent_score = torch.div(sent_hist.sum(dim=1), hist_news_size.to(self.device))
        sent_div_loss = F.relu(user_mean_sent_score.unsqueeze(dim=-1) * sent_cand * scores).mean()
        loss += self.hparams.sent_div_loss_coef * sent_div_loss
