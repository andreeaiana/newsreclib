...
import numpy as np
import torch
from lightning.pytorch.core.mixins import hparams_mixin
...
class DKNModule(AbstractRecommneder):
    ...
    Attributes:
        ...
        num_filters:
            The number of filters in the ``CNN`` of the news encoder.
        window_sizes:
            List of window sizes for the ``CNN`` of the news encoder.
        hidden_dim_dnn:
    ...
    def __init__(
        ...
        pretrained_entity_embeddings = torch.from_numpy(
            np.load(self.hparams.pretrained_entity_embeddings_path)
        ).float()
        pretrained_context_embeddings = torch.from_numpy(
            np.load(self.hparams.pretrained_entity_embeddings_path)
        ).float()

        # initialize news encoder
        self.news_encoder = NewsEncoder(
            pretrained_text_embeddings=pretrained_word_embeddings,
    ...
    def forward(self, batch: RecommendationBatch) -> torch.Tensor:
        ...
        # click scores
        scores = self.click_predictor(user_vector, cand_news_vector_agg.permute(0, 2, 1))
