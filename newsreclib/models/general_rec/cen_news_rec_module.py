...
from newsreclib.models.components.encoders.news.text import PLM, CNNMHSAAddAtt
from newsreclib.models.components.encoders.user.cen_news_rec import UserEncoder
from newsreclib.models.components.layers.click_predictor import DotProduct


class CenNewsRecModule(AbstractRecommneder):
    ...
    Attributes:
        ...
        dropout_probability:
            Dropout probability.
        gru_hidden_dim:
            The number of features in the hidden state of the ``GRU`` of the user encoder.
        num_recent_news:
    ...
    def __init__(
        ...
        if not self.hparams.late_fusion:
            self.user_encoder = UserEncoder(
                ...
                num_heads=self.hparams.num_heads,
                query_dim=self.hparams.query_dim,
                gru_hidden_dim=self.hparams.gru_hidden_dim,
                num_recent_news=self.hparams.num_recent_news,
                dropout_probability=self.hparams.dropout_probability,
            )
