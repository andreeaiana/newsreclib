...
from newsreclib.models.components.encoders.user.mins import UserEncoder
from newsreclib.models.components.layers.click_predictor import DotProduct


class MINSModule(AbstractRecommneder):
    ...
    def __init__(
        ...
        self.news_encoder = NewsEncoder(
            ...
            input_dim=self.hparams.text_embed_dim,
            query_dim=self.hparams.query_dim,
            output_dim=None,
        )
    ...
    def forward(self, batch: RecommendationBatch) -> torch.Tensor:
        ...
        if not self.hparams.late_fusion:
            # encode user
            user_vector = self.user_encoder(hist_news_vector_agg, hist_size)
        else:
